...
Para el Random Forest se realizó una búsqueda limitada de hiperparámetros utilizando únicamente el conjunto de entrenamiento. Con el objetivo de evitar fuga de información, esta búsqueda se llevó a cabo mediante una partición interna a nivel de partido, separando 2.217 partidos para el ajuste de los modelos candidatos y 555 partidos para la validación interna. De esta forma, todos los minutos pertenecientes a un mismo encuentro permanecen siempre dentro de la misma partición.

Se evaluaron cinco configuraciones distintas, modificando el número de árboles, la profundidad máxima, el número mínimo de muestras por hoja y el número de variables consideradas en cada división. En todos los casos se mantuvo el uso de pesos de clase balanceados para compensar el desbalance entre victoria local, empate y victoria visitante. La selección se realizó utilizando el F1-score macro como métrica principal, ya que esta permite valorar el rendimiento de forma equilibrada entre las tres clases.

La mejor configuración obtenida fue un Random Forest con 300 árboles, profundidad máxima igual a 12, un mínimo de 10 muestras por hoja y selección de variables mediante \texttt{log2}. Aunque algunas configuraciones obtenían una accuracy de validación ligeramente superior, esta configuración fue seleccionada porque alcanzaba el mejor F1-score macro, lo que indica un comportamiento más equilibrado entre clases.

\subsubsection{XGBoost}

Para XGBoost también se realizó una búsqueda de hiperparámetros utilizando únicamente el conjunto de entrenamiento. Al igual que en el caso del Random Forest, se creó una partición interna a nivel de partido, separando 2.217 partidos para el ajuste de los modelos candidatos y 555 partidos para la validación interna.

En la búsqueda se evaluaron cinco configuraciones distintas, modificando el número de estimadores, la profundidad máxima de los árboles, la tasa de aprendizaje, las tasas de submuestreo por filas y columnas, el peso mínimo por nodo hijo y el parámetro de regularización \texttt{reg\_lambda}. La selección se realizó utilizando el F1-score macro como métrica principal, ya que permite valorar el rendimiento de forma equilibrada entre victoria local, empate y victoria visitante.

La mejor configuración obtenida fue un XGBoost con 500 estimadores, profundidad máxima igual a 3, tasa de aprendizaje de 0,03, \texttt{subsample} de 0,8, \texttt{colsample\_bytree} de 0,8, \texttt{min\_child\_weight} igual a 5 y \texttt{reg\_lambda} igual a 10. Esta configuración obtuvo en la validación interna un accuracy del 65,0 \%, un F1-score macro del 62,5 \% y una pérdida logarítmica de 0,748. El resultado muestra que una configuración con árboles menos profundos, una tasa de aprendizaje menor y mayor regularización ofrece un comportamiento más estable y equilibrado para este problema.
...
El Random Forest obtiene un accuracy global del 65,5 \%, un F1-score macro del 62,9 \% y una pérdida logarítmica de 0,759 sobre el conjunto de test. Estos resultados indican que el modelo consigue un rendimiento sólido en la predicción del resultado final del partido, superando claramente la línea base basada en la clase mayoritaria. Además, el F1-score macro muestra que el rendimiento no se concentra únicamente en la clase más frecuente, sino que el modelo mantiene una capacidad razonable para distinguir entre victoria local, empate y victoria visitante.
...
El rendimiento temporal del modelo muestra una evolución coherente con la naturaleza del problema. En los primeros minutos, el modelo dispone de poca información acumulada sobre el desarrollo del encuentro, por lo que las predicciones resultan más inciertas. En el minuto 15 obtiene un accuracy del 54,2 \% y un F1-score macro del 49,2 \%, valores que reflejan la dificultad de anticipar el resultado final cuando el partido apenas ha comenzado.

A medida que avanza el encuentro, las métricas aumentan de forma progresiva. En el minuto 45, el modelo alcanza un accuracy del 62,7 \% y un F1-score macro del 59,8 \%, lo que indica que la información acumulada durante la primera parte ya aporta señales relevantes para la predicción. En el minuto 60, el modelo se aproxima al umbral del 70 \% de accuracy, con un valor del 69,4 \%, y en el minuto 75 lo supera claramente, alcanzando un 76,7 \% de accuracy y un 75,3 \% de F1-score macro. Finalmente, en el minuto 90, el rendimiento aumenta hasta un accuracy del 92,5 \% y un F1-score macro del 91,8 \%, debido a que el marcador y las métricas acumuladas representan ya de forma muy directa el desenlace del partido.

Esta evolución se observa en la figura \ref{fig:rf_minutos}, donde se aprecia un crecimiento sostenido tanto del accuracy como del F1-score macro a medida que avanza el encuentro.
...
La matriz de confusión, mostrada en la figura \ref{fig:rf_confusion}, permite analizar con más detalle el comportamiento del modelo para cada una de las clases. El modelo identifica correctamente una proporción similar de victorias locales y visitantes, con un recall del 72 \% en ambos casos. Esto indica que el Random Forest mantiene un comportamiento equilibrado en la detección de partidos que terminan con victoria de alguno de los dos equipos.

El empate continúa siendo la clase más difícil de predecir. Para esta clase, el modelo alcanza un recall del 44 \% y un F1-score del 45 \%. Este resultado es esperable en el contexto del fútbol, ya que el empate es una clase menos frecuente y especialmente sensible a pequeños cambios en el marcador. Un único gol en los minutos finales puede transformar un empate en victoria local o visitante, lo que dificulta que el modelo identifique esta clase con la misma estabilidad que las victorias.
...
El análisis de importancia de variables, mostrado en la figura \ref{fig:rf_importancia}, permite interpretar qué factores tienen mayor peso en las decisiones del modelo. La variable más relevante es la diferencia de goles, lo que resulta coherente con el problema planteado, ya que el marcador actual es uno de los indicadores más directos del resultado final. A continuación aparecen los goles acumulados del equipo local y visitante, así como variables relacionadas con la diferencia entre goles y xG y la diferencia total de xG entre equipos.

La presencia de variables como \texttt{xg\_diff}, \texttt{goals\_minus\_xg\_home} y \texttt{goals\_minus\_xg\_away} indica que el modelo no se limita únicamente a utilizar el marcador, sino que también incorpora información sobre la calidad de las ocasiones generadas y la relación entre rendimiento esperado y goles reales. Asimismo, variables como la posesión local y el total de goles aportan información contextual sobre el dominio del partido y el ritmo ofensivo del encuentro.
...
En conjunto, los resultados del Random Forest muestran que el modelo es capaz de aprovechar de forma efectiva las variables acumuladas minuto a minuto. Su rendimiento mejora de manera clara conforme avanza el partido, y las variables más importantes son coherentes tanto desde el punto de vista estadístico como futbolístico. No obstante, la dificultad para predecir empates evidencia que el problema sigue siendo complejo, especialmente en situaciones de partido equilibradas o con marcadores ajustados.

\subsubsection{XGBoost}

El modelo XGBoost obtiene un accuracy global del 65,7 \%, un F1-score macro del 63,2 \% y una pérdida logarítmica de 0,751 sobre el conjunto de test. Estos resultados muestran que el modelo alcanza un rendimiento sólido en la predicción del resultado final del partido y mantiene un comportamiento equilibrado entre las tres clases.
...
El rendimiento temporal del XGBoost muestra una evolución progresiva a medida que avanza el partido. En el minuto 15, el modelo obtiene un accuracy del 52,9 \% y un F1-score macro del 47,1 \%, lo que refleja la dificultad de anticipar el resultado final cuando todavía existe poca información acumulada sobre el desarrollo del encuentro. En estos primeros minutos, muchas variables relevantes, como el marcador, el xG o las acciones ofensivas, aún no han generado diferencias suficientemente claras entre ambos equipos.

A partir del minuto 30, el rendimiento empieza a mejorar de forma más notable. En el minuto 45, el modelo alcanza un accuracy del 63,7 \% y un F1-score macro del 61,4 \%, indicando que la información acumulada durante la primera parte ya permite diferenciar mejor entre los posibles resultados finales. En el minuto 60, el modelo se aproxima al umbral del 70 \% de accuracy, con un valor del 69,7 \%, y alcanza un F1-score macro del 68,2 \%. Posteriormente, en el minuto 75, supera claramente dicho umbral, con un accuracy del 76,3 \% y un F1-score macro del 75,0 \%. Finalmente, en el minuto 90, el modelo obtiene un accuracy del 92,9 \% y un F1-score macro del 92,3 \%, reflejando que el marcador y las métricas acumuladas representan ya de forma muy directa el desenlace del partido.

Esta evolución se muestra en la figura \ref{fig:xgb_minutos}, donde se observa el crecimiento progresivo del accuracy y del F1-score macro conforme se incorporan más minutos de información al modelo.
...
La matriz de confusión normalizada, mostrada en la figura \ref{fig:xgb_confusion}, permite analizar el comportamiento del modelo para cada una de las clases. El XGBoost identifica correctamente una proporción similar de victorias locales y visitantes, con un recall del 72 \% en ambos casos. Esto indica que el modelo captura de forma equilibrada muchos de los patrones asociados a situaciones en las que uno de los dos equipos termina imponiéndose.

La clase empate continúa siendo la más difícil de predecir, aunque el modelo consigue un rendimiento relevante para esta categoría, con un recall del 46 \% y un F1-score del 46 \%. Esta dificultad es coherente con la naturaleza del fútbol, donde los empates suelen depender de márgenes reducidos y pueden transformarse en victoria local o visitante por un único evento decisivo. Además, el empate funciona como una clase intermedia entre el dominio del equipo local y el dominio del equipo visitante, lo que dificulta su separación respecto al resto de clases.
...
El análisis de importancia de variables, mostrado en la figura \ref{fig:xgb_importancia}, permite identificar qué factores tienen mayor peso en las decisiones del modelo. La variable más relevante es la diferencia de goles, seguida de los goles acumulados del equipo local, los pases del equipo local y los goles acumulados del equipo visitante. También aparecen entre las variables más importantes las entradas en el tercio ofensivo del equipo visitante, las conducciones del equipo local, los tiros del equipo visitante, el total de goles, la diferencia de xG y la posesión local.

Estos resultados muestran que el XGBoost no se apoya únicamente en el marcador actual, sino que también utiliza variables relacionadas con el control del juego, el volumen ofensivo y la producción acumulada de ambos equipos. La presencia de variables como \texttt{passes\_home}, \texttt{attacks\_third\_away}, \texttt{carries\_home}, \texttt{shots\_away} y \texttt{xg\_diff} indica que el modelo incorpora información sobre dominio territorial, circulación de balón y generación de ocasiones, además del estado del marcador.
...
En conjunto, los resultados del XGBoost muestran que el modelo es capaz de aprovechar de forma efectiva las variables acumuladas minuto a minuto. Su rendimiento aumenta de manera clara conforme avanza el partido y las variables más importantes son coherentes desde el punto de vista futbolístico, ya que combinan información del marcador, el tiempo restante, la calidad ofensiva generada y ciertos elementos contextuales del juego.
